( function() {
	var canvas = document.getElementById( "demo_c5_depth" ).getElementsByTagName( "canvas" )[0];
	...
	gl.shaderSource( vertexShader, "attribute vec3 position; attribute vec2 texcoord; varying vec2 Texcoord; uniform mat4 model; uniform mat4 view; uniform mat4 proj; void main() { Texcoord = texcoord; gl_Position = proj * view * model * vec4( position, 1.0 ); }" );
	...
	// Set up view
	gl.uniformMatrix4fv( gl.getUniformLocation( shaderProgram, "view" ), false, mat4.lookAt( [ 1.5, 1.5, 1.5 ], [ 0, 0, 0 ], [ 0, 0, 1 ] ) );
