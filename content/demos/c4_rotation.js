( function() {
	var canvas = document.getElementById( "demo_c4_rotation" ).getElementsByTagName( "canvas" )[0];
	...
	gl.shaderSource( vertexShader, "attribute vec2 position; attribute vec2 texcoord; varying vec2 Texcoord; uniform mat4 trans; void main() { Texcoord = texcoord; gl_Position = trans * vec4( position, 0.0, 1.0 ); }" );
	...
	// Draw
	var uniTrans = gl.getUniformLocation( shaderProgram, "trans" );
	...
	registerAnimatedCanvas( canvas, function( time )
	{
		...
		gl.uniformMatrix4fv( uniTrans, false, mat );
